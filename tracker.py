...
class Vehicle (object):
    def __init__ (self, carid, position, rect, start_frame):
        self.id = carid
    ...
    def find_match (self, matches):
        ...
        for i, match in enumerate(matches):
            x, y, w, h = match
            position = (x+w//2, y+h)
            distance = self.get_distance(self.last_position, position)
            if self.is_valid_distance(distance):
                self.add_position(position, match)
                return i
    ...
    def calc_speed(self, frame_number, fps):
        ...
        pixels = self.get_distance(self.positions[-1], self.positions[-2])
    ...
    def draw (self, output_image):
        ...
        cv2.rectangle(output_image, (x, y), (x+w-1, y+h-1), (0, 255, 0))
    ...
    def track (self, matches, frame_number, output_image=None):
        ...
        for match in matches:	
            ...
            if position[0] < 100 or position[0] > 980: 
                new_vehicle = Vehicle(self.next_vehicle_id, position, match, frame_number)
                self.log.debug(frame_number, 'Add vehicle %d', self.next_vehicle_id, position[0], position[1])
